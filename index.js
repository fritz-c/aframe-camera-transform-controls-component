...
AFRAME.registerComponent('hint-scale', {
  ...
  init: function() {
    ...
    this.cameraObject = this.el.parentElement.querySelector(
      '[camera]'
    ).object3D;
...
AFRAME.registerComponent('camera-transform-controls', {
  ...
  twoHandInteraction: (function() {
    ...
    return function() {
      ...
      if (this.released) {
        ...
        midPoint
          .copy(currentPositionLeft)
          .add(currentPositionRight)
          .multiplyScalar(0.5);
        this.prevDistance = prevDistanceVec3
          .copy(currentPositionLeft)
      ...
      currentDistance = currentDistanceVec3
        .copy(currentPositionLeft)
        .sub(currentPositionRight)
        .length();
      ...
      centerVec3
        .copy(this.leftHandEl.object3D.position)
      ...
      this.hintEl.setAttribute('hint-scale', {
        text: this.cameraRigEl.object3D.scale.x.toFixed(2) + 'x',
      });
      ...
      this.cameraRigEl.setAttribute(
        'position',
        this.cameraRigEl.object3D.position
      );
  ...
  rotateScene: (function() {
    ...
    return function(midPoint, deltaAngle) {
      ...
      dirVec3
        .copy(cameraRigEl.object3D.position)
        .sub(midPoint)
...
AFRAME.registerComponent('camera-transform-controls-hand', {
  ...
  play: function() {
    this.el.sceneEl.components['camera-transform-controls'].registerHand(
      this.el,
      this.data.hand
    );
  },
